...
export default function App() {
  ...
  const [goal, setGoal] = useState<'lose_weight' | 'build_muscle' | 'maintain_weight'>('build_muscle');
  ...
  const handleValidate = () => {
    ...
    if (result.success) {
      ...
      setValidationMsg(`❌ Validation failed: ${result.error.errors[0].message}`);
    }
  };
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ border: '1px solid #ccc', padding: '1rem', marginTop: '1rem', borderRadius: '8px' }}>
        ...
        <label>
          Goal:
          <select value={goal} onChange={(e) => setGoal(e.target.value as 'lose_weight' | 'build_muscle' | 'maintain_weight')}>
